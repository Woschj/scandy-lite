...
from app.core.trash import purge_item, restore_item
...
async def test_purge_item_snapshots_closed_lending_and_deletes_item(session_maker, seed_data):
    async with session_maker() as session:
        ...
        worker = Worker(barcode="TRASH-W1", first_name="Max", last_name="Muster", department_id=seed_data["department_id"])
        session.add(item)
        session.add(worker)
        await session.commit()
...
async def test_purge_item_blocked_by_open_lending(session_maker, seed_data):
    async with session_maker() as session:
        ...
        worker = Worker(barcode="TRASH-W2", first_name="Erika", last_name="Muster", department_id=seed_data["department_id"])
        session.add(item)
        session.add(worker)
        await session.commit()
        await session.refresh(item)
